...
public class ClockView extends View {
	...
	private static final int DEFAULT_CLOCK_ANIMATION_DURATION = MINUTE;
	...
	public ClockView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mDigitalTimeTextSize = a.getDimensionPixelSize(R.styleable.ClockView_timeTextSize,
				dipToPx(context, DEFAULT_DIGITAL_TIME_TEXT_SIZE));
		...
		mClockPointRadius = a.getDimensionPixelSize(R.styleable.ClockView_pointRadius,
				dipToPx(context, DEFAULT_CLOCK_POINT_RADIUS));
		...
		mClockScaleLineWidth = a.getDimensionPixelSize(R.styleable.ClockView_timeScaleWidth,
				dipToPx(context, DEFAULT_CLOCK_SCALE_LINE_WIDTH));
		mClockScaleLineHeight = a.getDimensionPixelSize(R.styleable.ClockView_timeScaleHeight,
				dipToPx(context, DEFAULT_CLOCK_SCALE_LINE_HEIGHT));
		...
		mClockScaleLineColor = a.getDimensionPixelSize(R.styleable.ClockView_timeScaleColor,
				dipToPx(context, DEFAULT_CLOCK_SCALE_LINE_COLOR));
		a.recycle();
		init();
	}
	...
	private void init() {
		...
		mAdjustClockScaleLineStartX = dipToPx(getContext(), ADJUST_CLOCK_SCALE_LINE_START_X);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthSpecMode != MeasureSpec.EXACTLY) {
			widthMeasureSpec = MeasureSpec.makeMeasureSpec(
					dipToPx(getContext(), DEFAULT_CLOCK_VIEW_WIDTH), MeasureSpec.EXACTLY);
		}
	...
	private void updateTimeText(Canvas canvas) {
		...
		if (currentTimeMillis - mLastTimeMillis >= MINUTE) {
		...
		mClockAnimator.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationStart(Animator animation) {
				...
				mInitClockAngle = (mCalendar.get(Calendar.SECOND)
						+ (float) mCalendar.get(Calendar.MILLISECOND) / SECOND) * ANGLE_PER_SECOND;
				mLastTimeMillis = currentTimeMillis - mCalendar.get(Calendar.SECOND) * SECOND
